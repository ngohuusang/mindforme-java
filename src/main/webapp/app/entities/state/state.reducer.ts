...
export const getSearchEntities: ICrudSearchAction<IState> = (query, page, size, sort) => ({
  ...
  payload: axios.get<IState>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
...
export const getEntities: ICrudGetAllAction<IState> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IState>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};
