...
import { getSearchEntities, getEntities } from './child.reducer';
...
export const Child = (props: IChildProps) => {
  ...
  const { childList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="child-heading">
        Children
            ...
            <tbody>
              {childList.map((child, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{child.birthday ? <TextFormat type="date" value={child.birthday} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{child.status}</td>
                  <td>{child.relationName ? <Link to={`child-relation/${child.relationId}`}>{child.relationName}</Link> : ''}</td>
                  <td>{child.familyName ? <Link to={`family/${child.familyId}`}>{child.familyName}</Link> : ''}</td>
                  <td>{child.requestId ? <Link to={`help-request/${child.requestId}`}>{child.requestId}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${child.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${child.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
