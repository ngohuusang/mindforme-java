...
export const Pet = (props: IPetProps) => {
  ...
  const { petList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="pet-heading">
        Pets
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  Breed <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {petList.map((pet, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{pet.name}</td>
                  <td>{pet.imageUrl}</td>
                  <td>{pet.birthday ? <TextFormat type="date" value={pet.birthday} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{pet.status}</td>
                  <td>{pet.breedName ? <Link to={`pet-breed/${pet.breedId}`}>{pet.breedName}</Link> : ''}</td>
                  <td>{pet.petTypeName ? <Link to={`pet-type/${pet.petTypeId}`}>{pet.petTypeName}</Link> : ''}</td>
                  <td>{pet.familyName ? <Link to={`family/${pet.familyId}`}>{pet.familyName}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${pet.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${pet.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
