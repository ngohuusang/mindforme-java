...
import { getSearchEntities, getEntities } from './public-holiday.reducer';
...
export const PublicHoliday = (props: IPublicHolidayProps) => {
  ...
  const { publicHolidayList, match, loading, totalItems } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {publicHolidayList && publicHolidayList.length > 0 ? (
            ...
            <tbody>
              {publicHolidayList.map((publicHoliday, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{publicHoliday.day}</td>
                  <td>{publicHoliday.month}</td>
                  <td>{publicHoliday.year}</td>
                  <td>{publicHoliday.name}</td>
                  <td>
                    {publicHoliday.countryName ? <Link to={`country/${publicHoliday.countryId}`}>{publicHoliday.countryName}</Link> : ''}
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${publicHoliday.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${publicHoliday.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && <div className="alert alert-warning">No Public Holidays found</div>
        )}
      </div>
      {props.totalItems ? (
        <div className={publicHolidayList && publicHolidayList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ publicHoliday }: IRootState) => ({
  ...
  totalItems: publicHoliday.totalItems,
});
