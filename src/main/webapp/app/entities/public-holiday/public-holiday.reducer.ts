...
export const ACTION_TYPES = {
  SEARCH_PUBLICHOLIDAYS: 'publicHoliday/SEARCH_PUBLICHOLIDAYS',
...
export default (state: PublicHolidayState = initialState, action): PublicHolidayState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_PUBLICHOLIDAYS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_PUBLICHOLIDAYS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_PUBLICHOLIDAYS):
...
const apiUrl = 'api/public-holidays';
const apiSearchUrl = 'api/_search/public-holidays';

// Actions

export const getSearchEntities: ICrudSearchAction<IPublicHoliday> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_PUBLICHOLIDAYS,
  payload: axios.get<IPublicHoliday>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
...
export const getEntities: ICrudGetAllAction<IPublicHoliday> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IPublicHoliday>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};
