...
import { getSearchEntities, getEntities } from './emergency-contact.reducer';
    ...
    <div>
      ...
      <div className="table-responsive">
        {emergencyContactList && emergencyContactList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('relationToYou')}>
                  Relation To You <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {emergencyContactList.map((emergencyContact, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{emergencyContact.relationToYou}</td>
                  <td>{emergencyContact.phoneNumber}</td>
                  <td>{emergencyContact.email}</td>
                  <td>
                    {emergencyContact.addressAddress ? (
                      <Link to={`address/${emergencyContact.addressId}`}>{emergencyContact.addressAddress}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {emergencyContact.familyName ? (
                      <Link to={`family/${emergencyContact.familyId}`}>{emergencyContact.familyName}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && <div className="alert alert-warning">No Emergency Contacts found</div>
        )}
      </div>
      {props.totalItems ? (
