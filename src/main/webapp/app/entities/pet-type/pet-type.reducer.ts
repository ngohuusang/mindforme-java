...
export const getSearchEntities: ICrudSearchAction<IPetType> = (query, page, size, sort) => ({
  ...
  payload: axios.get<IPetType>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
...
export const getEntities: ICrudGetAllAction<IPetType> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IPetType>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};
