...
public class StateResourceIT {
     ...
     *
     * @see com.mindforme.app.repository.search.StateSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public static State createEntity(EntityManager em) {
        State state = new State().name(DEFAULT_NAME).status(DEFAULT_STATUS);
        return state;
    }
    ...
    public static State createUpdatedEntity(EntityManager em) {
        State state = new State().name(UPDATED_NAME).status(UPDATED_STATUS);
        return state;
    }
    ...
    public void createState() throws Exception {
        ...
        restStateMockMvc
            .perform(post("/api/states").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(stateDTO)))
    ...
    public void getAllStates() throws Exception {
        ...
        restStateMockMvc
            .perform(get("/api/states?sort=id,desc"))
    ...
    public void getState() throws Exception {
        ...
        // Get the state
        restStateMockMvc
            .perform(get("/api/states/{id}", state.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    public void getNonExistingState() throws Exception {
        // Get the state
        restStateMockMvc.perform(get("/api/states/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    public void updateState() throws Exception {
        ...
        updatedState.name(UPDATED_NAME).status(UPDATED_STATUS);
        StateDTO stateDTO = stateMapper.toDto(updatedState);

        restStateMockMvc
            .perform(put("/api/states").contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(stateDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteState() throws Exception {
        ...
        // Delete the state
        restStateMockMvc
            .perform(delete("/api/states/{id}", state.getId()).accept(MediaType.APPLICATION_JSON))
    ...
    public void searchState() throws Exception {
        ...
        // Search the state
        restStateMockMvc
            .perform(get("/api/_search/states?query=id:" + state.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(state.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
